...
class Trainer:
    ...
    def __init__(self, opt, class_list, train_dataloader, valid_dataloader, cl_mode=CL_NONE, tag=None, model=None):
        ...
        if self.model is None:
            if self.opt.model == "mfcc":
                self.model = MFCC_TCResnet(bins=40, channel_scale=self.opt.scale, num_classes=len(self.class_list))
            elif self.opt.model == "stft-mlp":
                self.model = STFT_MLP(filter_length=256, hop_length=129, bins=129, num_classes=len(self.class_list))
            else:
                # set the default model to STFT-ResNet.
                self.model = STFT_TCResnet(
                    filter_length=256, hop_length=129, bins=129,
                    channels=self.opt.cha, channel_scale=self.opt.scale, num_classes=len(self.class_list))
